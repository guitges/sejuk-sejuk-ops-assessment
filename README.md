...
- **"Postponed / Rescheduled" on the KPI dashboard has no data source to point at.** `weeklyPostponeCount()` looks for status-history notes containing "postpone", but no workflow in this build (admin, technician, or manager) ever creates one — there's no reschedule/postpone action anywhere in the UI. The metric is wired up and will populate correctly the moment such an action exists, but as shipped it will always read 0. I chose not to bolt on a fake "postpone" button just to make a number move; the other three KPI metrics (jobs completed, total amount, leaderboard) are fully real and demonstrate the aggregation/visualisation skill the brief asks for.
...
- **How AI tools were used while building this:** built end-to-end with Claude Code (Anthropic) — used to scaffold the React/Vite/Tailwind project, write the Supabase schema and RLS policies, implement all pages/modules, and drive an in-browser QA pass (creating orders, walking a job through Assigned → In Progress → Job Done → Reviewed, uploading a test file, and exercising the AI query module against the live deployment) that caught and fixed several real bugs before submission: an `order_no` sequence collision with seed data, a snake_case/camelCase mismatch that kept newly-assigned orders stuck on status "New", a missing SPA rewrite that 404'd on direct route access on Vercel, an initial design that would have put the AI provider's API key directly in client-side JS (moved to a serverless function instead once caught), AI answers getting silently truncated mid-sentence (a model-specific "thinking" token quirk eating the output budget) and a free-tier quota that turned out to be far too low for even light demo traffic, a shared header with no responsive wrapping that would have broken on the exact phone-width screens the technician persona is supposed to use, and a "notify manager when job completed" bonus that wrote to the database but was never actually surfaced anywhere in the UI.
